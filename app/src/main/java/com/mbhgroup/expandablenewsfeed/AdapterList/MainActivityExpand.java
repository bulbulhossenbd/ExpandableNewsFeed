...
import com.mbhgroup.expandablenewsfeed.R;
...
public class MainActivityExpand extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fedd_activity_main);
        mContext = this.getApplicationContext();
        mExpandableView = findViewById(R.id.expandableView);

        for (Feed feed : UtilsExpand.loadFeeds(this.getApplicationContext())) {
                            ...
                            if (!mIsLoadingMore
                                ...
                                new Handler(Looper.getMainLooper()).post(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        for(Feed feed : UtilsExpand.loadFeeds(getApplicationContext())){
                                            mExpandableView.addView(new HeadingView(mContext, feed.getHeading()));
                                            for(Info info : feed.getInfoList()){
                                                mExpandableView.addView(new InfoView(mContext, info));
                                            }
